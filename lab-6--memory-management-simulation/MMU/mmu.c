...
int main(int argc, char *argv[]) 
{
   ...
   list_t *FREE_LIST = list_alloc();   // all free blocks (PID is always zero)
   list_t *ALLOC_LIST = list_alloc();  //all allocated blocks
   int i;
   ...
   for(i = 0; i < N; i++) // simulates a memory management policy
   {
       printf("************************\n");
